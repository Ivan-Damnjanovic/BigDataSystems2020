...
def prediction(input_data, regressor, model, output_attribute_index):
    ...
    scikit_prediction_vector = regressor.predict(input_matrix)
    ...
    # Ispisivanje prave vrednosti zajedno sa prediktovanim vrednostima
    for row in zip(output_vector, scikit_prediction_vector, spark_prediction_vector):
        print("Real: %10f\tScikit: %10f\tSpark: %10f" % (row[0], row[1], row[2][0]))

    # Racunaju se root mean squared error metrike i ispisuju na konzoli
    metrics_scikit = mean_squared_error(output_vector, scikit_prediction_vector)
    metrics_spark = mean_squared_error(output_vector, spark_prediction_vector)
    print("Scikit-learn - Root mean squared error: " + str(math.sqrt(metrics_scikit)))
